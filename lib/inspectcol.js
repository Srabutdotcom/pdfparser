function fillcol(row,xMax,xMin){
    ...
    for(let i=0;i<row.length;i+=1){
      ...
      let xF1 = parseFloat(row[i].x.toFixed(1));
      let rF1 = parseFloat(row[i].r.toFixed(1));
      dcol.push({
        left:[xF1],
        x:xF1,
        r:rF1,
  ...
  updatecol=(l,s,dirX,left,right)=>{
    ...
    for(let i=0;i<l.length;i+=1){
      ...
      if(l[i].x!==s[i].x) {//both x must be reside in i
          let tmp = Math.min(l[i].x*dirX,s[i].x*dirX);
          if(!tmp){
            console.log(`something wrong with ${l[i]} and ${s[i]} we need to bypass this`)
            debugger;continue;
          }
          l[i].x = tmp;
          let indexOfx = l[i]['left'].indexOf(tmp);
          if(indexOfx===-1){
            l[i]['left'].push(l[i].x)
          } else {
            if(!left.has(tmp)){
      ...
      if(l[i].r!==s[i].r&&s[i].r*dirX<(l?.[i+1]?.x??l['xMax'])*dirX){
        ...
        let tmp = Math.max(l[i].r*dirX,s[i].r*dirX);
        if(!tmp){
            console.log(`something wrong with ${l[i]} and ${s[i]} we need to bypass this`);
            debugger;continue;
          }
        l[i].r = tmp;
        let indexOfr = l[i]['right'].indexOf(tmp);
        if(indexOfr===-1){
